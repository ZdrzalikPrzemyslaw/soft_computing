...
void train_all_epoch(int size, int count, int epoch_count, double *neuron, double *inputs, double training_step, activation_function f);
...
int main(int argc, char const *argv[])
{
    ...
    struct all_data all_data = {
        ...
        .training_patterns = gen_training_patterns(in_data.N, in_data.M, in_data.training_in_min, in_data.training_in_max)};

    printf("%f\n", neuron_output(in_data.N, all_data.neuron, all_data.training_patterns, &lineral_func));

    train_all_epoch(in_data.N, in_data.M, in_data.epoch_count, all_data.neuron, all_data.training_patterns, in_data.training_step, &lineral_func);
...
void train_all_patterns(int size, int count, double *neuron, double *inputs, double training_step, activation_function f)
{
    for (int i = 0; i < count; i++)
    {
